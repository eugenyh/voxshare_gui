...
def get_default_config():
    ...
    return {
        "gui": {
            ...
            "main_geometry": "550x450" # Увеличили размер для списка пиров
        },
        "network": {
...
class AudioTransceiver:
    def __init__(self, net_config, audio_config):
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                if addr[0] == self.local_ip:
                    # logging.debug("Пакет от себя проигнорирован.") # Убираем лишний лог
                    continue

                # --- ИСПРАВЛЕНИЕ: Проверка типа пакета через startswith() ---
                if data.startswith(self.packet_type_audio):
                    ...
                    try:
                        ...
                        logging.debug(f"Аудио пакет ({len(opus_packet)} байт от {addr[0]}) добавлен в очередь.")
                    except queue.Full:
                        try:
                            dropped_tuple = self.received_opus_packets.get_nowait()
                            self.received_opus_packets.put_nowait(packet_tuple)
                            logging.warning(f"Очередь аудио пакетов полна. Отброшен пакет от {dropped_tuple[0]}.")
                        except queue.Empty: pass
                        except queue.Full:
                             logging.warning("Очередь полна даже после удаления, новый пакет пропущен.")

                elif data.startswith(self.packet_type_ping):
                     # Проверяем, что пакет это ТОЛЬКО пинг (правильной длины)
                     if len(data) == len(self.packet_type_ping):
                         with self.clients_lock:
                             self.active_clients[addr[0]] = time.time()
                         # Используем INFO, чтобы видеть в логе по умолчанию
                         logging.info(f"Получен PING от {addr[0]}. Клиент активен.")
                     else:
                         # Логируем, если пришел "PING" с лишними данными
                         logging.warning(f"Получен пакет, начинающийся с PING, но с неверной длиной ({len(data)} байт) от {addr}.")
                else:
    ...
    def cleanup_inactive_clients(self):
        ...
        with self.clients_lock: active_client_ips = list(self.active_clients.keys()) # Копия для итерации
        for ip in active_client_ips:
             with self.clients_lock: last_seen = self.active_clients.get(ip)
             if last_seen is None: continue # Если удалили между шагами
             if current_time - last_seen > client_timeout: inactive.append(ip)
        if inactive:
            logging.debug(f"Кандидаты на удаление: {inactive}")
            with self.clients_lock:
                for ip in inactive:
...
class AudioSelector(ctk.CTk):
    ...
    def validate_and_launch(self):
        ...
        try:
            ...
            self.destroy()
            main_app = VoxShareGUI(input_device_index=self.input_device_index, output_device_index=self.output_device_index, gui_config=self.gui_config, net_config=config.get('network',{}), audio_config=config.get('audio',{}))
            main_app.mainloop()
...
class VoxShareGUI(ctk.CTk):
    def __init__(self, input_device_index, output_device_index, gui_config, net_config, audio_config):
        ...
        try: self.audio_transceiver = AudioTransceiver(net_config=self.net_config, audio_config=self.audio_config)
    ...
    def setup_gui(self):
        ...
        # Вложенный фрейм для размещения текста в строку
        text_frame = ctk.CTkFrame(top_frame, fg_color="transparent")  # Без фона
        text_frame.pack()

        # Жирный VOX
        bold_label = ctk.CTkLabel(text_frame, text="VOX", font=("Arial", 24, "bold"))
        bold_label.pack(side="left", anchor="s", pady=1)

        # Тонкий SHARE
        thin_label = ctk.CTkLabel(text_frame, text="SHARE", font=("Arial", 24, "normal"))
        thin_label.pack(side="left", anchor="s", pady=0)
        
        # ctk.CTkLabel(top_frame, text="VOXSARE", font=("Arial", 24)).pack()

        # --- Средняя часть (Лого + Список пиров) ---
        ...
        # --- Нижняя часть (Вертикальное расположение) ---
        bottom_frame = ctk.CTkFrame(self, fg_color="transparent"); bottom_frame.pack(side="bottom", pady=10, padx=10, fill="x")
        bottom_frame.columnconfigure(0, weight=1) # Одна колонка для растягивания

        # LED индикатор (row 0)
        self.led = ctk.CTkLabel(bottom_frame, text="", width=30, height=30, corner_radius=15, fg_color="#D50000")
        self.led.grid(row=0, column=0, padx=10, pady=(5, 2)) # Центрируется

        # Индикатор громкости (row 1)
        self.volume_bar = ctk.CTkProgressBar(bottom_frame, height=20)
        self.volume_bar.set(0)
        self.volume_bar.grid(row=1, column=0, padx=50, pady=2, sticky="ew") # Растягиваем с отступами

        # Кнопка "Говорить" (row 2)
        self.talk_btn = ctk.CTkButton(bottom_frame, text="Говорить", height=40, font=("Arial", 16))
        self.talk_btn.grid(row=2, column=0, padx=50, pady=(5, 5), sticky="ew") # Растягиваем с отступами
    ...
    def update_gui(self):
        ...
        try:
            ...
            active_peers = []; current_speaker = None; now = time.time()
            try:
                with self.audio_transceiver.clients_lock: active_peers = sorted(list(self.audio_transceiver.active_clients.keys()))
            except AttributeError: logging.warning("update_gui: audio_transceiver не найден при доступе к clients_lock."); return
            with self.speaker_lock:
                if self.currently_speaking_ip and (now - self.last_packet_played_time > SPEAKER_TIMEOUT_THRESHOLD): logging.debug(f"Сброс статуса говорящего для {self.currently_speaking_ip} по таймауту."); self.currently_speaking_ip = None
                if self.currently_speaking_ip and self.currently_speaking_ip not in active_peers: logging.debug(f"Говорящий {self.currently_speaking_ip} больше не в списке активных пиров."); self.currently_speaking_ip = None
                current_speaker = self.currently_speaking_ip
            if hasattr(self, 'peer_list_textbox') and self.peer_list_textbox.winfo_exists():
                try:
                     # last_index = self.peer_list_textbox.index("end-1c") # Скролл пока не работает надежно
                     self.peer_list_textbox.configure(state="normal")
                     self.peer_list_textbox.delete("1.0", "end")
                     if active_peers:
                         for peer_ip in active_peers: prefix = "* " if peer_ip == current_speaker else "  "; self.peer_list_textbox.insert("end", f"{prefix}{peer_ip}\n")
                     else: self.peer_list_textbox.insert("end", " (нет других участников)")
                     self.peer_list_textbox.configure(state="disabled")
                     # self.peer_list_textbox.see(last_index)
                except tkinter.TclError as e: logging.warning(f"Ошибка обновления peer_list_textbox (возможно, виджет уничтожен): {e}")
